...
describe('tests for <SpaceListMeeting />', () => {
  ...
  it('should render attendees prop(node property)', () => {
    const container = mount(
      <SpaceListMeeting
        header="header"
        attendees={[{ title: 'Joe Bojangles', node: <div className="internalNode" /> }]}
      />
    );

    expect(
      container.find('.md-event-overlay__children').children().find('.internalNode').length
    ).toEqual(0);
  ...
  it('should handle buttonOnClick and execute keyPress', () => {
    ...
    container.find('.md-button').simulate('keyDown', { which: 13, charCode: 13, key: 'Space' });
    expect(onClick).toHaveBeenCalled();
    expect(onClickParent).not.toHaveBeenCalled();
  });
  ...
  it('should pass props to ListItem', () => {
    ...
    const container = mount(
      <SpaceListMeeting customAnchorNode={customAnchorNode} header="header" />
    );
  ...
  describe('tests for title Prop', () => {
    it('should not have title by default if header is node', () => {
      const container = mount(<SpaceListMeeting header={<div>test</div>} />);

      expect(container.find('.md-list-item').props().title).toEqual(undefined);
    });

    it('should handle title prop', () => {
      const container = mount(<SpaceListMeeting header="header" title="testTitle" />);

      expect(container.find('.md-list-item').props().title).toEqual('testTitle');
    });

    it('should handle title if header is string', () => {
      const container = mount(<SpaceListMeeting header="testTitle" />);
