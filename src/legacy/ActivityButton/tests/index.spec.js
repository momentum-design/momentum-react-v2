...
describe('tests for <ActivityButton />', () => {
  ...
  it('should handle keyDown as onClick event', () => {
    ...
    container.find('Button').simulate('click');
    container.find('Button').simulate('keyDown', { which: 13, charCode: 13, key: 'Space' });
  ...
  it('should render custom activity', () => {
    const container = mount(
      <ActivityButton
        ariaLabel="test"
        type={{ color: 'red', icon: <span className="icon icon-arrow-left_32" /> }}
      />
    );
    ...
    expect(
      container
        .find('button')
        .hasClass('md-button md-button--circle md-button--68 md-button--red md-activity')
    ).toEqual(true);
  });
});
