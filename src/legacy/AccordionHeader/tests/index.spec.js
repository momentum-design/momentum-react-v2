...
import { AccordionHeader } from '@momentum-ui/react-collaboration';
...
describe('tests for <AccordionHeader />', () => {
  ...
  it('when the showSeparator prop is true, should display the separator', () => {
    ...
    expect(
      container.find('.md-accordion__header').hasClass('md-accordion__header--separator')
    ).toEqual(true);
  });
  ...
  it('when height prop is set, should apply height style to header', () => {
    ...
    expect(container.find('.md-accordion__header').hasClass('md-accordion__header--56')).toEqual(
      true
    );
  });
