...
class DeviceListCall extends React.PureComponent {
  ...
  render() {
    const { className, devices, header, ...props } = this.props;
    const { selectedIndex } = this.state;

    const otherProps = omit({ ...props }, ['defaultSelected', 'onSelect']);

    const getLeftSection = (deviceType) => {
      switch (deviceType) {
        case 'device':
          return <Icon name="spark-board_20" />;
        default:
          return <Icon name="laptop_20" />;
      }
    };
    ...
    return (
      ...
      >
        ...
        {devices.map(({ eventKey, id, name, title, type, value, ...deviceProps }, idx) => (
            ...
            {(uid) => {
              ...
              return (
                <ListItem
                  ...
                  {...deviceProps}
                >
                  <ListItemSection position="left">{getLeftSection(type)}</ListItemSection>
                  <ListItemSection position="center">
                    <div className="md-list-item__header">{name}</div>
                  </ListItemSection>
